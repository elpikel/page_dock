defmodule PageDockWeb.SiteLive.Index do
  ...
  # Per-site tile gradients, chosen deterministically from the slug.
  @tiles [
    ...
    >
      ...
      <div :if={@site_count > 0} id="sites" phx-update="stream" class="flex flex-col gap-2.5">
        <div
          ...
          id={dom_id}
          class="card flex items-center gap-4 p-4 hover:border-border2 transition-colors"
        >
          <span class={[
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2.5">
              <.link
                ...
                class="text-[14.5px] font-semibold tracking-[-0.01em] text-text truncate no-underline hover:underline"
              >
                {site.name}
              </.link>
            ...
            <div class="text-[12.5px] font-mono text-muted truncate">
              {Sites.public_domain(site)}
            </div>
          </div>

          <div class="hidden md:flex items-center gap-2 shrink-0 max-w-[280px] text-[12.5px] font-mono text-muted">
            <svg class="w-3.5 h-3.5 shrink-0 text-faint" viewBox="0 0 16 16" fill="currentColor">
            ...
            <span class="shrink-0 px-1.5 py-px rounded-[5px] bg-raised border border-border text-[11.5px]">
              {site.default_branch}
            </span>
          </div>

          <span class="hidden sm:block shrink-0 text-[13px] text-muted whitespace-nowrap">
          ...
          <div class="flex items-center gap-1 shrink-0">
            <a
              ...
              rel="noopener"
              title="Open site"
              class="w-8 h-8 rounded-md text-faint hover:text-text hover:bg-raised inline-flex items-center justify-center"
            >
            ...
            <.link
              ...
              id={"delete-#{site.id}"}
              title="Delete site"
              class="w-8 h-8 rounded-md text-faint hover:text-bad hover:bg-raised inline-flex items-center justify-center cursor-pointer"
            >
      ...
      >
        <span class="w-6 h-6 rounded-[7px] mr-2.5 text-white inline-flex items-center justify-center text-[16px] leading-none bg-[linear-gradient(135deg,#5B5FE8,#8A63F0)]">
          +
        </span>
  ...
  defp tile_gradient(slug), do: Enum.at(@tiles, rem(:erlang.phash2(slug), 5))

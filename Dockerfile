...
#     E.g.: docker.io/hexpm/elixir:1.18.1-erlang-27.3-debian-trixie-20260610-slim
...
# Run migrations on every boot, then start the server. The `&&` means a failed
# migration aborts startup, so a broken schema never serves traffic (Ecto's
# migration lock makes running it on every restart safe). Matches `check_signature`.
